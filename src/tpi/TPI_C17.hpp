...
public:
		...
		for (size_t i = 0; i < pisUnordered.size(); i++) {
			for (Levelized* pi : c17->pis()) {
				...
				if (piName == order.at(i)) {
					pisOrdered.push_back(dynamic_cast<SimulationNode<VALUE>*>(pi));
					break;
				}
			}
		}
	...
	Parser<LINETYPE, NODETYPE, VALUE> parser;
	Circuit* c17 = parser.Parse("c17.bench");
	TPI_COP<NODETYPE, LINETYPE, VALUE> tpi = TPI_COP<NODETYPE, LINETYPE, VALUE>(c17);
	TPGenerator<TPCONTROL, NODETYPE, LINETYPE, VALUE> controlGen;
	TPGenerator<TPOBSERVE, NODETYPE, LINETYPE, VALUE> observeGen;
	TPGenerator<TPINVERT, NODETYPE, LINETYPE, VALUE> invertGen;
	std::set<TP*> controlTPs = controlGen.allTPs(c17);
	std::set<TP*> observeTPs = observeGen.allTPs(c17);
	std::set<TP*> invertTPs = invertGen.allTPs(c17);

	std::unordered_set<Levelized*> pisUnordered = c17->pis();
	std::vector<SimulationNode<VALUE>*> pisOrdered;

	VALUE o = VALUE(0);
	VALUE i = VALUE(1);
	VALUE x = VALUE();
	std::vector<VALUE> v3 = { i,o,i,o,o }; //Is is known that this vector will detect 6 of 22 collapsed faults:
...
TEST_F(C17Tests, C17ExhaustiveControl) {
	for (TP* controlTP : controlTPs) {
		...
		printf("\nDBG TP %s\n", tpName.c_str());
		float foundFC = tpi.quality(controlTP, c17);
...
TEST_F(C17Tests, C17ExhaustiveObserve) {
	for (TP* observeTP : observeTPs) {
		...
		printf("\n\nDBG TP: %s\n",tpName.c_str());
		float foundFC = tpi.quality(observeTP, c17);
		float expFC = ExpOneVecFC.at(tpName);
		EXPECT_NEAR(foundFC, expFC, 0.0001);
	}
}
...
TEST_F(C17Tests, C17ExhaustiveInvert) {
	for (TP* invertTP : invertTPs) {
		...
		tpName = tpName + "_" + lineName + fanout;
		printf("\n\nDBG TP: %s\n", tpName.c_str());
		float foundFC = tpi.quality(invertTP, c17);
		float expFC = ExpOneVecFC.at(tpName);
		EXPECT_NEAR(foundFC, expFC, 0.0001);
	}
}
...
TEST_F(C17Tests, C17ExhaustiveObserveFaultCoverage) {
	for (TP* observeTP : observeTPs) {
		...
		FaultSimulator<VALUE> faultSimulator;
		FaultGenerator<VALUE> faultGenerator;
		std::unordered_set<Fault<VALUE>*> faults = faultGenerator.allFaults(c17);
		faultSimulator.setFaults(faults);
		observeTP->activate(c17);
		faultSimulator.applyStimulus(c17, v3, EventQueue<VALUE>(), pisOrdered);
		...
		//Cleanup
		faultSimulator.setFaults(std::unordered_set<Fault<VALUE>*>());
...
TEST_F(C17Tests, C17ExhaustiveControlFaultCoverage) {
	for (TP* controlTP : controlTPs) {
		...
		eventQueue.add(controlTP->activate(c17));
		faultSimulator.applyStimulus(c17, v3, eventQueue, pisOrdered);
		...
		//Cleanup
		faultSimulator.setFaults(std::unordered_set<Fault<VALUE>*>());
		eventQueue.add(controlTP->deactivate(c17));
		std::vector<VALUE> clearVector = std::vector<VALUE>({x, x, x, x, x});
		faultSimulator.applyStimulus(c17, clearVector, EventQueue<VALUE>(), pisOrdered);
	}
}