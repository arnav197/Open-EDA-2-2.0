...
public:
	void SetUp() override {
		...
		for (size_t i = 0; i < pisUnordered.size(); i++) {
			for (Levelized* pi : c17->pis()) {
				...
				if (piName == order.at(i)) {
					c17PisOrdered.push_back(dynamic_cast<SimulationNode<Value<bool>>*>(pi));
					break;
				}
			}
		}
		...
		for (size_t i = 0; i < pisUnordered.size(); i++) {
			for (Levelized* pi : c17copy->pis()) {
				...
				std::string piName = piLine->name();
				if (piName == order.at(i)) {
					c17CopyPisOrdered.push_back(dynamic_cast<SimulationNode<Value<bool>>*>(pi));
					break;
				}
			}
		}
	...
	std::vector<SimulationNode<Value<bool>>*> c17PisOrdered;
	std::vector<SimulationNode<Value<bool>>*> c17CopyPisOrdered;

	Simulator<Value<bool>> simulator;
...
TEST_F(CopyTest, c17CopyTest) {
	...
	do {
		...
		bool mismatch = ValueVectorFunction<Value<bool>>::mismatch(originalOut, copyOut);
		EXPECT_EQ(false, mismatch);
	} while (ValueVectorFunction<Value<bool>>::increment(inputVector));
}
