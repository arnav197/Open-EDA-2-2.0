...
public:
		...
		for (size_t i = 0; i < pisUnordered.size(); i++) {
			for (Levelized* pi : c->pis()) {
				...
				if (piName == order.at(i)) {
					pisOrdered.push_back(dynamic_cast<SimulationNode<Value<unsigned long long int>>*>(pi));
					break;
				}
			}
		}
	...
	Parser<SimulationLine<Value<unsigned long long int>>, SimulationNode<Value<unsigned long long int>>, Value<unsigned long long int>> parser;
	Circuit* c = parser.Parse("c17.bench");
	std::unordered_set<Levelized*> pisUnordered = c->pis();
	std::vector<SimulationNode<Value<unsigned long long int>>*> pisOrdered;
	...
	Simulator<Value<unsigned long long int>> sim;
	EventQueue<Value<unsigned long long int>> simulationQueue;
...
TEST_F(WideSimulatorTest, c17_exhaustive) {
	for (size_t repeat = 0; repeat < 100; repeat++) { //It was found that fails can be intermittent.
		...
		std::vector<Value<unsigned long long int>> response = sim.applyStimulus(
			c,
			inputs,
			EventQueue<Value<unsigned long long int>>(),
			pisOrdered
		);
